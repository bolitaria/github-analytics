...
def get_test_token():
    """Generate a JWT token for testing (bypasses real authentication)."""
    payload = {
        "user_id": "test_user",
        "role": "admin",
...
def test_api_predictions_endpoint(api_client, setup_database):
    """Test /api/repos/*/predictions endpoint."""
    # Ensure predictions exist (call training if needed)
    try:
        from src.models.forecast import train_and_forecast, save_predictions
    ...
    response = api_client.get(
        "/api/repos/test/integration/predictions",
...
def test_api_repos_endpoint(api_client, setup_database):
    ...
    data = json.loads(response.data)
    assert isinstance(data, list)
    # Our test repo should appear
    repos = [repo.get("name") for repo in data]
    assert "test/integration" in repos
