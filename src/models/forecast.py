...
def train_and_forecast(repo_name: str, periods: int = 30):
    ...
    for _, row in forecast.iterrows():
        predictions.append({
            ...
            'predicted_events': int(max(0, row['yhat'])),  # Aseguramos que no sea negativo
...
def save_predictions(predictions: list):
    ...
    VALUES
    """
    values = [(p['repository'], p['forecast_date'], p['predicted_events'], 
               p['lower_bound'], p['upper_bound'], p['model_type'], p['training_date']) 
