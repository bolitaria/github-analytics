...
class GitHubETL:
    ...
    def fetch_events(self, owner: str, repo: str, since: Optional[datetime] = None) -> List[dict]:
    ...
    def _generate_demo_events(self, owner: str, repo: str, since: Optional[datetime] = None) -> List[dict]:
        """Generate demo events for development"""
        event_types = ["PushEvent", "PullRequestEvent", "IssuesEvent", "WatchEvent", "ForkEvent", "CreateEvent"]
        users = ["alice-dev", "bob-contributor", "carol-maintainer", "dave-reviewer", "eve-bot"]
    ...
    def transform_event(self, event_data: dict) -> GitHubEvent:
        ...
        try:
            ...
            actor_login = event_data['actor']['login'] if isinstance(event_data['actor'], dict) else event_data['actor']
            repo_name = event_data['repo']['name'] if isinstance(event_data['repo'], dict) else event_data['repo']
            ...
            if event_data.get('org'):
                org_login = event_data['org']['login'] if isinstance(event_data['org'], dict) else event_data['org']
    ...
    def load_events(self, events: List[GitHubEvent]):
        ...
        for event in events:
            data.append({
                ...
                'payload': json.dumps(event.payload),  # Convert dict to JSON string for storage
    ...
    def run_etl(self, owner: str, repo: str, days_back: int = 7):
        ...
        try:
            ...
            logger.info(f"ETL completed for {owner}/{repo}. Processed {len(transformed_events)} events")
    ...
    def generate_sample_data(self, count: int = 100):
        ...
        event_types = ['PushEvent', 'IssuesEvent', 'WatchEvent', 'PullRequestEvent', 'ForkEvent']
